...
int axp313_set_poweroff_sequence(void)
{
	...
	ret = axp313_write(AXP313A_PWROK_VOFF_SERT, reg);
	if (ret)
		return ret;

	ret = axp313_read(AXP313A_POWER_DOMN_SEQUENCE, &reg);
	if (ret)
		return ret;
	reg |= (1 << 4);//enable reset PMIC by pull down PWROK 0x1A bit 4
	ret = axp313_write(AXP313A_POWER_DOMN_SEQUENCE, reg);
	if (ret)
		return ret;
